...
void    render_map(t_data *data)
{
    int width = 64;
    int height = 64;
    data->wall = mlx_xpm_file_to_image(data->mlx, "textures/wall.xpm", &height, &width);
    data->player = mlx_xpm_file_to_image(data->mlx, "textures/player.xpm", &height, &width);
    data->tiles = mlx_xpm_file_to_image(data->mlx, "textures/tiles.xpm", &height, &width);
    data->coin = mlx_xpm_file_to_image(data->mlx, "textures/coin.xpm", &height, &width);
    data->door = mlx_xpm_file_to_image(data->mlx, "textures/door1.xpm", &height,&width);
    data->dooropen = mlx_xpm_file_to_image(data->mlx, "textures/door2.xpm", &height, &width);
    if( data->door == NULL || data->player == NULL || data->tiles == NULL || data->coin == NULL || data->dooropen  == NULL || data->wall == NULL)
    {
        ft_free1(data->map);
        handletextures(data);
        ...
        while (data->map[i][j])
        {
            ...
            else if(data->map[i][j] == 'E')
            {
                if(calculatecoin(data) == 0)
                    mlx_put_image_to_window(data->mlx, data->win, data->dooropen , j* 64, i * 64);
                else
